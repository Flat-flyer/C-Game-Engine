...
void MouseEventListener::NotifyOfMousePressed(int butonType_)
{
	if (engineInstance) {
		engineInstance->NotifyOfMousePressed(mouse, butonType_);
	}
}
...
{
	if (engineInstance) {
		engineInstance->NotifyOfMouseReleased(mouse, butonType_);
	}
}
